...
/* TRANSLATOR Gui::Dialog::AddonManager */

AddonManager::AddonManager(QWidget* parent, Qt::WindowFlags fl)
  : QDialog(0, fl|Qt::WindowStaysOnTopHint), viewAxis(0,0,-1), ui(new Ui_AddonManager)
{
    // create widgets
    ui->setupUi(this);
...
void AddonManager::accept()
{
    reset();
    QDialog::accept();
}

void AddonManager::reject()
{
    reset();
    QDialog::reject();
}

bool AddonManager::eventFilter(QObject *obj, QEvent *event)
{
...
void AddonManager::showEvent(QShowEvent *)
{
    if (this->wasHidden)
        this->move(this->pnt);
...
void AddonManager::hideEvent(QHideEvent *)
{
    this->pnt = this->pos();
    this->wasHidden = true;
...
Gui::View3DInventor* AddonManager::activeView() const
{
...
float AddonManager::getSpeed(int v) const
{
    float speed = ((float)v)/50.0f; // let 2.0 be the maximum speed
    return speed;
}

SbVec3f AddonManager::getDirection(Gui::View3DInventor* view) const
{
...
void AddonManager::on_angleSlider_valueChanged(int v)
{
...
void AddonManager::reorientCamera(SoCamera * cam, const SbRotation & rot)
{
...
void AddonManager::on_speedSlider_valueChanged(int v)
{
    Q_UNUSED(v); 
    Gui::View3DInventor* view = activeView();
...
void AddonManager::on_playButton_clicked()
{
    Gui::View3DInventor* view = activeView();
    if (view) {
        if (!view->getViewer()->isAnimating()) {
...
void AddonManager::on_stopButton_clicked()
{
    Gui::View3DInventor* view = activeView();
    if (view)
...
void AddonManager::on_fullscreen_toggled(bool on)
{
    Gui::View3DInventor* view = activeView();
    if (view) {
...
void AddonManager::on_timeout_valueChanged(int v)
{
    timer->setInterval(v*1000);
}

void AddonManager::onAutoPlay()
{
    Gui::View3DInventor* view = activeView();
    if (view && !view->getViewer()->isAnimating()) {
        startAnimation(view);
...
void AddonManager::startAnimation(Gui::View3DInventor* view)
{
...
void AddonManager::on_timerCheck_toggled(bool on)
{
    if (on)
        timer->start();
    else
...
void AddonManager::changeEvent(QEvent *e)
{
